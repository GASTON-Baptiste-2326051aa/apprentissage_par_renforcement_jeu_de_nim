...
class WriteExcel:
    def __init__(self, matches, take, default_count, reward, punishment, path, name):
        ...
        self.nb_self.take = len(self.take)
        self.games_offset = 7
        self.game_height = 2 + 2*self.nb_self.take
        
        self.workbook = xlsxwriter.Workbook(path)
        self.worksheet = self.workbook.add_self.worksheet(name)
    ...
    def setup(self):
        glob_titles = ["max allumettes", "coups possibles", "proba initiale de chaque coup", "nb billes/couleur", "récompense", "punishment"]
        glob_values = [self.matches, self.take, self.default_proba, self.default_count, self.reward, self.punishment]
        ...
        self.worksheet.write(5, states_offset-1, "état", self.glob_titles_format)
        for i in range(self.matches):
            self.worksheet.write(5, states_offset+i, 8-i, self.glob_titles_format)
    ...
    def add_game(self, results, steps_p1, steps_p2, cups_p1, cups_p2, cups_reseted_p1 = [], cups_reseted_p2 = []):
        ...
        self.worksheet.merge_range(
            first_row+2, 1,
            first_row+1+self.nb_self.take, 1,
            "P1", self.games_titles_format
        )
        self.worksheet.merge_range(
            first_row+2+self.nb_self.take, 1,
            first_row+1+self.nb_self.take*2, 1,
        ...
        self.worksheet.merge_range(
            first_row+2, 2,
            first_row+1+self.nb_self.take, 2,
            results["P1"] if "P1" in players else "no data", self.games_titles_format
        )
        self.worksheet.merge_range(
            first_row+2+self.nb_self.take, 2,
            first_row+1+self.nb_self.take*2, 2,
